...
public class ClientController {
	...
	public String addClient(@Valid Client client, BindingResult bindingResult, Model model,
		...
		if (bindingResult.hasErrors()) {
			...
			attributes.addFlashAttribute("success", "customer saved successfully");
			return "redirect:/views/clients/";
		}
	}

	@GetMapping("/edit/{idclient}")
	public String edit(@PathVariable Long idclient, Model model, RedirectAttributes attributes) {
		...
		if (idclient > 0) {
			...
			if (client == null) {
				attributes.addFlashAttribute("error", "ATENTION:Customer ID does not exist !!!!");
				return "redirect:/views/clients/";
			}
		} else {
			attributes.addFlashAttribute("error", "ERROR: ERROR Customer with ID :<");
			return "redirect:/views/clients/";
		}
	...
	public String delete(@PathVariable Long idclient, RedirectAttributes attributes) {
		Client client = null;
		...
		attributes.addFlashAttribute("warning", "WARNING:record successfully removed");
		return "redirect:/views/clients/";
	}
}
